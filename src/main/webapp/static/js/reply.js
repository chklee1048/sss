...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 댓글 번호 쉽게 알 수 있다. 와 이거 정말 너무하네
    // 콜백 함수 : 삭제라는 버튼을 눌렀을 때 호출
    // 호출 이라 함은 클릭 이벤트 일어남. 이벤트 타겟 찾자.(-> 버튼)
    // 클릭 일어난 타겟 찾음
    // data-id 라는 속성 찾음  
    const deleteReply = function(e){
    ...
    // 댓슬 수정 모달 객체 생성
    // backdrop true:모달창 바깥 클릭하면 사라짐 false:취소 x 아닌이상 안 사라짐
    ...
    const showUpdateModal = (e) =>{
        ...
        const reqUrl = `/spring2/api/reply/${id}`; 
        // 모달창 textarea 내용 노출 요청을 Ajax 방식으로 보냄
        axios.get(reqUrl)
            .then((response)=>{
                ...
                //modalReplyText.value = response.data.replyText;
                // 모달을 보여줌.
                modal.show();    
            })
            .catch((error)=>{
    ...
    const updateReply = (e) =>{
        ...
        // Ajax 요청에 대한 성공/실패 callback 등록.
        // 수정 버튼을 클릭 했을 때, 저장 버튼을 눌렀을 때 일어나는 일
        // 
    ...
    /*modalBtnUpdate.addEventListener('click', (e)=>{});
    modalBtnUpdate.addEventListener('click', function(){
    ...
    const makeReplyElements = (data) => {
        ...
        for (let reply of data) {
            ...
            <div class="card">
                <div>
                    ...
                    <span class="text-secondary fs-small">${modified}</span>
                </div>
                <div>
                    ${reply.replyText}
                </div>
        ...
        /*미리 만들어진 html 아니라서 이 문자열이 만들어지고 나서 버튼 이벤트 해야 됨
                id도 반복문 안에 있어서 이 버튼을 구분할 방법을 찾아야 한다.*/ 
        
        // 작성된 HTML 코드를 replies <div> 영역 안에 포함.
        replies.innerHTML = htmlStr;
        
        // 모든 삭제 버튼들을 찾아서 클릭 이벤트 리스너를 등록
        // 모든 버튼 찾을 때, 우리 클래스로 찾자
